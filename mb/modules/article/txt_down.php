...
//增加章节列表开始
        $bookid = $_GET['id'];
        $sql3 = mysql_query("SELECT * FROM `jieqi_article_chapter` WHERE `articleid` = $bookid AND `chaptertype` != 1 order by `chapterid` desc limit 0,30");
        while($list = mysql_fetch_array($sql3,1)){
            $listdd[] = $list;
        }
		$t = 1;
        $subdir = jieqi_getsubdir($bookid);
       if(is_array($listdd)){
			foreach($listdd as $v){
				$sjc_time = $v[lastupdate];     
				$date_format = date('m月d日H:i',$sjc_time);  
				
					//$sup_chapter .= "<div><a href='/modules/article/wapreader.php?aid=".$bookid."&cid=".$v[chapterid]."'>".$v[chaptername]."</a></div>";
				if($t < 4){
					$sup_chapter .= "<li class='new_chapter'><a href='/".intval($bookid/1000)."/".$bookid."/".$v[chapterid].".html' target='_blank'>$v[chaptername]</a><i>".$t."</i></li>";   
				}
				else{
					$sup_chapter .= "<li><a href='/".intval($bookid/1000)."/".$bookid."/".$v[chapterid].".html' target='_blank'>$v[chaptername]</a><i>".$t."</i></li>";				
				}   
        ...
        $jieqiTpl->assign( "last9", $sup_chapter );
        //增加章节列表结束
        ...
        $jieqiTset['jieqi_contents_template'] = $jieqiModules['article']['path'].'/templates/txt_down.html';
