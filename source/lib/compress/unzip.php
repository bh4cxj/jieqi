 ...
 * @author     $Author: juny $
 * @version    $Id: unzip.php 197 2008-11-25 03:00:05Z juny $
 */

/*
用法
$file="./test.zip"; //要解压的文件
$target_dir = "./test"; //解压到哪个目录
$pack_manager = new JieqiUnzip($file); //创建解压类
$pack_manager->ExtractZip($target_dir); //开始解压
...
class JieqiUnzip{
	...
	function ExtractZip(){
		...
		if ($v_size > 0) {
			...
			if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
				$v_result = $this->ParseOptions($v_arg_list, $v_size, $v_options,
				array (77001 => 'optional',
				77003 => 'optional',
				77004 => 'optional',
				77002 => 'optional',
				78001 => 'optional',
		...
		$p_list = array();
		$v_result = $this->ExtractByRule($p_list, $v_path, $v_remove_path,
	...
	function ParseOptions(&$p_options_list, $p_size, &$v_result_list, $v_requested_options=false){
		$v_result=1;
		$i=0;
		while ($i<$p_size) {
	...
	function CheckFormat($p_level=0){
		$v_result = true;
		clearstatcache();
	...
	function ExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
	{
		$v_result=1;
		...
		for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++){
			...
			if ((isset($p_options[77008]))&&($p_options[77008] != 0)) {
				for ($j=0; ($j<sizeof($p_options[77008])) && (!$v_extract); $j++) {
					if (substr($p_options[77008][$j], -1) == "/") {
						if (   (strlen($v_header['stored_filename']) > strlen($p_options[77008][$j]))
						&& (substr($v_header['stored_filename'], 0, strlen($p_options[77008][$j])) == $p_options[77008][$j])) {
							$v_extract = true;
						}
			...
			}else if ((isset($p_options[77010]))&&($p_options[77010] != "")) {
				if (ereg($p_options[77010], $v_header['stored_filename'])) {
					$v_extract = true;
				}
			}else if ((isset($p_options[77011]))&& ($p_options[77011] != "")) {
				if (preg_match($p_options[77011], $v_header['stored_filename'])) {
					$v_extract = true;
				}
			}else if ( (isset($p_options[77009]))&& ($p_options[77009] != 0)) {
	...
	function CloseFd(){
		//--(MAGIC-PclTrace)--//PclTraceFctStart(__FILE__, __LINE__, "PclZip::CloseFd", "");
		$v_result=1;
	...
	function ConvertHeader2FileInfo($p_header, &$p_info){
		$v_result=1;
	...
	function ExtractFileInOutput(&$p_entry, &$p_options){
		$v_result=1;
		if (($v_result = $this->ReadFileHeader($v_header)) != 1) {
			return $v_result;
		}
	...
	function DirCheck($p_dir, $p_is_dir=false){
		$v_result = 1;
	...
	function ExtractFileAsString(&$p_entry, &$p_string){
		$v_result=1;
		$v_header = array();
		if (($v_result = $this->ReadFileHeader($v_header)) != 1)
		{
	...
	function CheckFileHeaders(&$p_local_header, &$p_central_header){
		$v_result=1;
	...
	function PathInclusion($p_dir, $p_path){
		$v_result = 1;
		if (   ($p_dir == '.')
	...
	function SwitchWinPath($p_path, $p_remove_disk_letter=true){
