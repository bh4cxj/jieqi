...
if($_GET['confirm'] != 1){
	echo '<font color="red">注意：</font><br>本程序用于数据库的更新，适合从JIEQI CMS 1.4X 升级到 1.50 版。数据库升级之前请做好备份，以免意外情况无法恢复！<br><br><a href="'.basename($_SERVER['PHP_SELF']).'?confirm=1">点击这里开始更新数据库</a><br><br>';
	exit;
}
...
ob_flush();
flush();
$sql="UPDATE `jieqi_system_modules` SET version=150 WHERE name='article';
UPDATE `jieqi_system_modules` SET version=150 WHERE name='forum';
UPDATE `jieqi_system_modules` SET version=130 WHERE name='obook';
UPDATE `jieqi_system_modules` SET version=120 WHERE name='cartoon';";
$sqlary=array();
jieqi_splitsqlfile($sqlary, $sql);
foreach($sqlary as $v){
	$v=trim($v);
