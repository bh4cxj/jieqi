...
$errtext='';
if($db_query->connect($_POST['mysql_host'], $_POST['mysql_user'], $_POST['mysql_pass'], $_POST['mysql_name'], false)){
	//创建数据库
	if($_POST['setup_table']=='yes'){
		//判断数据库名称合法性
		if(preg_match('/^[A-Za-z_]+[A-Za-z0-9_]*$/', $_POST['mysql_name'])){
	...
	$errtext.=$jieqiLang[JIEQI_MODULE_NAME]['mysql_connect_failure'].'<br />';
}
if(empty($errtext)){
	$link_status=1;
	//纪录注册账号会话
