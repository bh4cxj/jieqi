 ...
 * @author     $Author: juny $
 * @version    $Id: forummanage.php 331 2009-02-13 09:07:14Z juny $
 */
...
if(!is_object($forum)){
	...
	switch($_REQUEST['action']){
		...
		$errtext='';
		if (strlen($_POST['forumname'])==0) $errtext .= $jieqiLang['forum']['need_forum_name'].'<br />';
		if(empty($errtext)) {
			...
			if($_POST['master'] != ''){
			    ...
			    if($qstr != ''){
					...
					while($v = $users_handler->getObject($res)){
						...
						if(strlen($masterary[$i]['uname']) == 0) $masterary[$i]['uname'] = $v->getVar('uname', 'n');
						$i++;
					}
			...
			if(!$forums_handler->insert($forum)) jieqi_printfail($jieqiLang['forum']['save_forum_failure']);
			else{
		...
		if(is_array($masterary) && count($masterary)>0){
		    foreach($masterary as $v){
		        if($v['uname'] != ''){
		            if(!empty($masterstr)) $masterstr.=' ';
		            $masterstr.=$v['uname'];	
		        }	
		    }
		}
		...
		$viewary=unserialize($forum->getVar('authview','n'));
		if(!is_array($viewary)) $viewary=array();
		$authview=new JieqiFormCheckBox($jieqiLang['forum']['auth_view'], 'authview', $viewary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authview->addOption($k, $v);
		}
		$edit_form->addElement($authview);

		$readary=unserialize($forum->getVar('authread','n'));
		if(!is_array($readary)) $readary=array();
		$authread=new JieqiFormCheckBox($jieqiLang['forum']['auth_read'], 'authread', $readary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authread->addOption($k, $v);
		}
		$edit_form->addElement($authread);

		$postary=unserialize($forum->getVar('authpost','n'));
		if(!is_array($postary)) $postary=array();
		$authpost=new JieqiFormCheckBox($jieqiLang['forum']['auth_post'], 'authpost', $postary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authpost->addOption($k, $v);
		}
		$edit_form->addElement($authpost);

		$replyary=unserialize($forum->getVar('authreply','n'));
		if(!is_array($replyary)) $replyary=array();
		$authreply=new JieqiFormCheckBox($jieqiLang['forum']['auth_reply'], 'authreply', $replyary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authreply->addOption($k, $v);
		}
		$edit_form->addElement($authreply);

		$uploadary=unserialize($forum->getVar('authupload','n'));
		if(!is_array($uploadary)) $uploadary=array();
		$authupload=new JieqiFormCheckBox($jieqiLang['forum']['auth_upload'], 'authupload', $uploadary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authupload->addOption($k, $v);
		}
		$edit_form->addElement($authupload);

		$editary=unserialize($forum->getVar('authedit','n'));
		if(!is_array($editary)) $editary=array();
		$authedit=new JieqiFormCheckBox($jieqiLang['forum']['auth_edit'], 'authedit', $editary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authedit->addOption($k, $v);
		}
		$edit_form->addElement($authedit);

		$deleteary=unserialize($forum->getVar('authdelete','n'));
		if(!is_array($deleteary)) $deleteary=array();
		$authdelete=new JieqiFormCheckBox($jieqiLang['forum']['auth_delete'], 'authdelete', $deleteary);
		foreach($jieqiGroups as $k => $v){
			if($k != JIEQI_GROUP_ADMIN) $authdelete->addOption($k, $v);
		}
