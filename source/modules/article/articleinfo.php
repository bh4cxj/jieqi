...
$jieqiTpl->assign('user_gold', $userarray[egold]);
	...
	$jieqiTpl->assign('flower', $article->getVar('flower'));
	$jieqiTpl->assign('egg', $article->getVar('egg'));
	...
	*/
	...
	$sql1 = mysql_query("SELECT chapterid FROM `jieqi_article_chapter` WHERE `articleid` = $_REQUEST[id] and chaptertype != 1 order by `chapterid` asc limit 1");
    $zhangjieId = mysql_fetch_object($sql1);
    $jieqiTpl->assign( "firstzhangjieId", $zhangjieId->chapterid );

	$sql3 = mysql_query("SELECT * FROM `jieqi_article_chapter` WHERE `articleid` = $_REQUEST[id] and chaptertype != 1 order by `chapterid` desc limit 6");
        while($list = mysql_fetch_array($sql3,1)){
            $listdd[] = $list;
        }

        $jieqiTpl->assign( "indexrows", $listdd );
    //增加粉丝榜
	$sql12 = "SELECT u.`name`,sum(t.nums) shu,t.dateline,t.uid FROM jieqi_article_tip as t
		...
		ORDER BY shu desc
		LIMIT 10";
	$tipData = mysql_query($sql12);
	while($lists = mysql_fetch_array($tipData,1)){
		$listdds[] = $lists;
	}

    $jieqiTpl->assign( "tiprows", $listdds );
	//增加粉丝榜结束
	$author = $article->getvar( "author" );
	$authsql = mysql_query("SELECT `articleid`,`articlename` FROM `jieqi_article_article` WHERE `author` = '$author' order by `lastupdate` desc limit 10");
	while($authrow = mysql_fetch_array($authsql,1)){
		if($bookid != $authrow[articleid]){
			$authother[] = $authrow;
		}
	}

    $jieqiTpl->assign( "authother", $authother );
