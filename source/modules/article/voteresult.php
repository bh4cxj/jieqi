...
if(!$voteres) jieqi_printfail($jieqiLang['article']['avote_not_exists']);
...
for($i=1;$i<=$useitem;$i++){
	...
	if($statall > 0) $resultrows[$i-1]['percent']=sprintf("%0.2f", $voteres->getVar('stat'.$i) * 100 / $statall);
	else $resultrows[$i-1]['percent']=0;
