...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			progress.setStatus("文件超过 " + this.settings['file_size_limit'] + "限制大小.");
			this.debug("错误提示: 文件超过限制大小, 文件名: " + file.name + ", 文件大小: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			progress.setStatus("文件大小不能为0.");
			this.debug("错误提示: 文件大小为0, 文件名: " + file.name + ", 文件大小: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			progress.setStatus("文件类型错误.");
			this.debug("错误提示: 文件类型错误, 文件名: " + file.name + ", 文件大小: " + file.size + ", Message: " + message);
			break;
		default:
			if (file !== null) {
				progress.setStatus("未知错误");
			}
			this.debug("错误提示: " + errorCode + ", 文件名: " + file.name + ", 文件大小: " + file.size + ", Message: " + message);
			break;
		}
	} catch (ex) {
        this.debug(ex);
...
function uploadSuccess(file, serverData) {
	try {
		...
		progress.toggleCancel(false);//alert('a');
...
function queueComplete(numFilesUploaded) {
	...
	status.innerHTML = numFilesUploaded + " 个文件" + (numFilesUploaded === 1 ? "" : "")+"已经上传.";
}
